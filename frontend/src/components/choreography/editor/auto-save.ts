...
export function startAutoSave(
  save: (data: { id: string; title: string; layout: { elements: Array<{ code: string; timestamp: number; goe: number }> } }) => void,
  isPending: () => boolean,
  delay = 500,
): () => void {
  ...
  return useChoreographyEditor.subscribe(
    (state) => {
      ...
      timer = setTimeout(() => {
        ...
        save({
          id: pid,
          title: state.title,
          layout: { elements: layout.map((el) => ({ code: el.code, timestamp: el.timestamp, goe: el.goe })) },
        })
      }, delay)
    },
  )
