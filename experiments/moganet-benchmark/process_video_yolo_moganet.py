#!/usr/bin/env python3
"""
Process video with YOLOv8 detection + MogaNet-B pose estimation.
Pipeline: frame -> YOLO detect persons -> crop each person -> MogaNet-B -> draw skeleton.
"""
...
def crop_from_bbox(img, bbox, padding=0.2):
    """Crop image from bbox with padding."""
    h, w = img.shape[:2]
    cx, cy, bw, bh = bbox
    x1 = max(0, int(cx - bw / 2 - bw * padding))
    y1 = max(0, int(cy - bh / 2 - bh * padding))
    x2 = min(w, int(cx + bw / 2 + bw * padding))
    y2 = min(h, int(cy + bh / 2 + bh * padding))
...
def process_video(video_path, output_path, backbone, head, device):
    ...
    with torch.no_grad():
        while True:
            ...
            # Draw each person
            for bbox in bboxes:
                crop, (x1, y1, x2, y2) = crop_from_bbox(frame, bbox, padding=0.2)
                if crop.size == 0:
                    continue
                crop_h, crop_w = crop.shape[:2]
                crop_aspect = crop_w / crop_h

                # Resize to 384x288 maintaining aspect ratio
                if crop_aspect > 384 / 288:
                    new_w = 384
                    new_h = int(384 / crop_aspect)
                else:
                    new_h = 288
                    new_w = int(288 * crop_aspect)

                img_resized = cv2.resize(crop, (new_w, new_h))
                img_rgb = cv2.cvtColor(img_resized, cv2.COLOR_BGR2RGB)

                # Pad to 384x288
                pad_img = np.zeros((288, 384, 3), dtype=np.uint8)
                pad_img[:new_h, :new_w] = img_rgb
                ...
                heatmaps = head(feats[3])
                heatmaps_np = heatmaps[0].cpu().numpy()

                # Decode
                keypoints, scores = decode_heatmap_simple(heatmaps_np, (384, 288), (96, 72))

                # Scale keypoints back to crop size
                scale_x = crop_w / new_w
                scale_y = crop_h / new_h
                keypoints[:, 0] *= scale_x
                keypoints[:, 1] *= scale_y

                # Translate back to original frame coords
                keypoints[:, 0] += x1
                keypoints[:, 1] += y1

                # Draw skeleton on original frame
                frame = draw_skeleton(frame, keypoints, skeleton)
            ...
            if frame_idx % 30 == 0:
                ...
                print(
                    f"  Frame {frame_idx}/{total_frames} | persons: {len(bboxes)} | avg FPS: {avg_fps:.1f}"
                )
            frame_idx += 1
    ...
    print(f"Processed {processed} frames in {total_time:.1f}s ({avg_fps:.1f} FPS)")
...
def main():
    ...
    print("Loading MogaNet-B model...")
    backbone, head = load_model("/root/moganet_b_ap2d_384x288.pth", device)
    print("MogaNet-B loaded.")

    video_path = "/root/videos/parnoe.MOV"
    output_path = "/root/videos/parnoe_yolo_moganet.mp4"
