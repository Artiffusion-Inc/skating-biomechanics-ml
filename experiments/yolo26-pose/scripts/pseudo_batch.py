...
def get_heatmap_maximum(heatmaps: np.ndarray):
    ...
    # locs[vals <= 0.] = -1  # Removed: regression heatmaps have negative values
    return locs, vals
...
for video_dir in tqdm(frame_dirs, desc="Processing videos"):
    ...
    for frame_path in frame_files:
        ...
        W, H = 96, 72
# DISABLED: refine_keypoints_dark_udp broken - using heatmap maximum only
#        keypoints = refine_keypoints_dark_udp(keypoints, heatmaps_np, blur_kernel_size=11)  # DISABLED: breaks regression heatmaps
        # Clamp to heatmap bounds to prevent out-of-bounds
#        # Clip removed - breaks invalid keypoints (-1 -> 0)
        ...
        for i in range(17):
            ...
            # Check if keypoint is on image boundary (suspicious)
            on_boundary = (x <= 1 or x >= w - 2 or y <= 1 or y >= h - 2)

            # Score threshold with boundary penalty
            if on_boundary:
                # Boundary keypoints are unreliable - max v=1
                v = 1.0 if scores[i] > 0.0005 else 0.0
            elif scores[i] > 0.001:
