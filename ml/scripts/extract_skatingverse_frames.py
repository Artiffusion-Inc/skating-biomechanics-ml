...
def extract_frames(video_path: Path, output_dir: Path, fps: int = 10) -> dict:
    ...
    try:
        ...
        return {"video_id": video_id, "num_frames": 0, "status": "error", "error": str(e)}


def main():
    parser = argparse.ArgumentParser(description="Extract frames from SkatingVerse videos")
    parser.add_argument(
        "--video-dir", type=Path, required=True, help="Path to SkatingVerse test_videos directory"
    )
    parser.add_argument(
        "--output-dir", type=Path, required=True, help="Path to output frames directory"
    )
    parser.add_argument(
        "--fps", type=int, default=10, help="Frames per second to extract (default: 10)"
    )
    parser.add_argument(
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        futures = {
            executor.submit(extract_frames, video_path, args.output_dir, args.fps): video_path
        ...
        json.dump(
            {
                ...
                "summary": {
                    ...
                    "avg_frames_per_video": total_frames / success_count
                    if success_count > 0
                    else 0,
                },
            },
